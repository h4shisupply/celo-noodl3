...
- Confirm dynamic visit QR countdowns expose the remaining duration as machine-readable time.
...
- Confirm malformed or expired dynamic visit QR links show clear user-facing error states.
...
- Confirm reward ticket QR renders locally, exposes the printable reward ticket sheet, and shows the counter backup code prominently.
- Confirm the shop owner wallet can validate each reward ticket only once.
...
- Confirm browser-visible `NEXT_PUBLIC_` RPC URLs point to public endpoints and server-side RPC endpoints stay in server-only variables.
...
- Confirm QR scanner live regions announce complete scanner status changes.
...
- Confirm root metadata keywords cover Celo, Celo loyalty, MiniPay, PWA, loyalty, loyalty program, customer loyalty, customer QR stamp card, merchant loyalty, merchant QR stamp card, QR, counter QR, QR stamp card, stamp card, QR code, printed visit QR, static visit QR, dynamic visit QR, live visit QR, live QR, visit QR, wallet stamps, visit stamps, reward ticket, reward ticket QR, printable reward ticket sheet, merchant rewards, and merchant reward surfaces.
...
- Confirm mobile and desktop layouts for these routes:
  ...
  - `/app/claim/[claimId]`
- Confirm camera QR scanning works with HTTPS and localhost URLs.
- Confirm print output includes only the active printed visit QR counter sheet or printable reward ticket sheet.
...
- Confirm legacy `/merchant/verify` and `/verify` links without a claim query redirect to the `/app` dashboard.
