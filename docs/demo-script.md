...
Use separate owner and customer wallets or separate browser profiles so wallet-gated states stay clear during the demo. Confirm `NEXT_PUBLIC_APP_URL` matches the local, LAN, or deployed URL that phones will scan before showing QR links.
...
3. Open `/app`, connect a wallet, optionally add a display name, and create a merchant QR stamp card with a square public HTTPS logo image URL, reward promise, and customer visit goal.
...
11. Open `/app/claim/[claimId]`, show the reward ticket QR, reward ticket sheet, counter backup code, and shop owner wallet validation hint.
12. Switch back to the owner wallet, validate the reward ticket, refresh the claim page, and confirm that a second validation fails.
