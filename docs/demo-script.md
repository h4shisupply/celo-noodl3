...
Use separate shop owner and customer wallets, or use separate browser profiles, so wallet-gated states stay easy to distinguish during the demo. Confirm `NEXT_PUBLIC_APP_URL` matches a local, LAN, or deployed URL phones can reach before showing QR links.
...
6. Generate a live visit QR from an HTTPS URL or localhost URL, show the five-minute countdown, scan it as a customer, and collect one visit stamp instantly.
...
9. Collect or issue enough visit stamps to reach the configured visit goal.
10. Create the reward ticket from the customer QR stamp card.
11. Open `/app/claim/[claimId]` and show the reward ticket QR, printable reward ticket sheet, counter backup code, and shop owner wallet validation guidance.
