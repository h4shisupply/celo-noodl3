...
For QR testing from phones on the same local network, use `npm run dev:mobile` so the dev server listens on `0.0.0.0`.
Camera QR scanning requires HTTPS or a localhost URL.
