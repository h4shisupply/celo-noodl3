...
- Accessible app structure with named landmarks, visible focus, progress meters, timer labels, full language names for language controls, live regions, QR scanner feedback, and scanner busy states
...
- Manual stamp issuance for shop owner wallets
...
- `/app/claim/[claimId]`: reward ticket QR, reward ticket sheet, counter backup code, and one-time shop owner wallet validation
- `/claim/[claimId]`: legacy reward ticket link that preserves the claim ID when redirecting to `/app/claim/[claimId]`
- `/app/rewards`, `/rewards`, `/success`, `/app/store/[slug]`, and `/store/[slug]` redirect to the `/app` dashboard; `/merchant/verify` and `/verify` preserve `claim` query values when redirecting to `/app/claim/[claimId]`, and links without a `claim` query value redirect to `/app`
...
- Every static visit, dynamic visit, or manual stamp issuance gives exactly one stamp.
...
Camera-based QR scanning works over HTTPS or on localhost, so use a secure tunnel or HTTPS preview for phone QR tests against a LAN dev server.
