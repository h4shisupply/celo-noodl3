...
- `/`: public landing page
- `/app`: customer and owner dashboard page for customer and merchant QR stamp cards, visit stamps, and reward ticket history
...
- `/app/rewards`, `/rewards`, `/success`, `/app/store/[slug]`, and `/store/[slug]` redirect into the `/app` dashboard page; `/merchant/verify` and `/verify` preserve `claim` query values when redirecting into `/app/claim/[claimId]`, or into `/app` otherwise
...
After the dev server starts, open `http://localhost:3000/app` to view customer QR stamp cards, visit stamps, and reward ticket history on the `/app` dashboard page.
