      ...
      <Card className="max-h-[calc(100dvh-3rem)] w-full max-w-md overflow-y-auto">
        <CardHeader className="space-y-3">
          <div className="flex items-start justify-between gap-4">
            <div
              className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg border border-accent-border bg-accent-soft text-accent"
              aria-hidden="true"
            >
            ...
            <button
              type="button"
              className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-line bg-panel text-muted transition hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-focus"
              onClick={onClose}
              aria-label={`${dictionary.common.close}: ${dictionary.profile.title}`}
              title={`${dictionary.common.close}: ${dictionary.profile.title}`}
            >
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-3">
            <Button
              ...
              title={isSaving ? dictionary.profile.saving : dictionary.actions.save}
            ...
            <Button
              variant="ghost"
              title={dictionary.actions.skipForNow}
              onClick={handleSkip}
              disabled={isSaving}
            >
