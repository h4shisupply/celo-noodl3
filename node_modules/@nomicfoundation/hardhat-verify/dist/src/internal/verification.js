...
import { filterVersionsByRange, resolveSupportedSolcVersions, } from "./solc-versions.js";
...
Explorer: ${instance.getContractUrl(address)}`);
    ...
    const compatibleSolcVersions = filterVersionsByRange(supportedSolcVersions, deployedBytecode.solcVersion);
    ...
    const { success: minimalInputVerificationSuccess } = await attemptVerification({
...
`);
    ...
    const { success: fullCompilerInputVerificationSuccess, message: verificationMessage, } = await attemptVerification({
        verificationProvider: instance,
        address,
        encodedConstructorArgs,
        creationTxHash,
...
export function validateVerificationProviderName(provider) {
    if (Object.keys(VERIFICATION_PROVIDERS).indexOf(String(provider)) === -1) {
        throw new HardhatError(HardhatError.ERRORS.HARDHAT_VERIFY.VALIDATION.INVALID_VERIFICATION_PROVIDER, {
